...
The demo seeder drives 10 governed-AI scenarios through the live app. Each turn is **safe and synthetic** — PII/PHI, toxicity, and hallucination signals come from the app's own `force_*` injection switches, not real content.
...
Every turn is **server-assigned** a shared `request_id` / `trace_id` / `gen_ai.event.id`. The whole story — Cisco Agent Observability quality score, AI Defense verdict, PII/injection/hallucination scores, latency, cost — joins on it.
...
**Soft reset (between sessions — keep the environment running):** see [Wrap-Up](wrap-up.html#reset-between-sessions-soft-reset).
...
5. Log out of the four consoles if on shared hardware.

{: .note }
> The historical demo data in `index=gen_ai_log` persists by design — it powers the rolling 7–30 day proof points. You don't need to delete it between sessions.
